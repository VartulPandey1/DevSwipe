...
userRouter.get("/user/requests", userAuth, async (req, res) => {
  try {
    ...
    const data = await connection.find({
      ...
      status: "interested",
    }).populate("connectionRequestFrom", "firstName lastName age photoURL skills gender about");
    res.send(data)
  } catch (err) {
...
userRouter.get("/user/connections", userAuth, async (req, res) => {
  try {
    ...
    const data = await connection
      ...
      .populate("connectionRequestTo", "firstName lastName age photoURL skills gender about")
      .populate("connectionRequestFrom", "firstName lastName age photoURL skills gender about");
    res.send(data);
  } catch (err) {
    res.send(err.message);
